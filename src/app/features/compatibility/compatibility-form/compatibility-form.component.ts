...
export class CompatibilityFormComponent implements OnInit {
  ...
  loadAvailableOrgans(): void {
    this.organsService.getAvailableOrgans().subscribe({
      ...
      error: () => {
        this.snackBar.open('Error loading available organs', 'Close', {
          duration: 5000,
        });
      },
    });
  ...
  loadWaitingReceivers(): void {
    this.receiversService.getWaitingReceivers().subscribe({
      ...
      error: () => {
        this.snackBar.open('Error loading waiting receivers', 'Close', {
          duration: 5000,
        });
      },
    });
  ...
  createCompatibilityRecord(): void {
    this.compatibilityService
      .createCompatibilityRecord(this.compatibilityForm.value)
      .subscribe({
        next: () => {
          this.snackBar.open(
            'Compatibility record created successfully',
            'Close',
            { duration: 5000 }
          );
          this.router.navigate(['/compatibility']);
        ...
        error: (error) => {
          this.snackBar.open(
            error.error?.message || 'Error creating compatibility record',
            'Close',
            { duration: 5000 }
          );
          this.isSaving = false;
  ...
  updateCompatibilityRecord(): void {
    ...
    this.compatibilityService
      .updateCompatibilityRecord(
        ...
        this.compatibilityForm.value
      )
      .subscribe({
        next: () => {
          this.snackBar.open(
            'Compatibility record updated successfully',
            'Close',
            { duration: 5000 }
          );
          this.router.navigate(['/compatibility']);
        ...
        error: (error) => {
          this.snackBar.open(
            error.error?.message || 'Error updating compatibility record',
            'Close',
            { duration: 5000 }
          );
          this.isSaving = false;
