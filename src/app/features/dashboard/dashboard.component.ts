...
export class DashboardComponent implements OnInit, AfterViewInit {
  @ViewChild('organTypeChart') organTypeChart!: BaseChartDirective;
  @ViewChild('bloodTypeChart') bloodTypeChart!: BaseChartDirective;
  ...
  bloodTypeChartData: ChartData = {
    labels: [],
    datasets: [{ data: [], label: 'Receivers by Blood Type', backgroundColor: [] }]
  };
  ...
  loadDashboardData(): void {
    ...
    forkJoin({
      overview: this.http.get(`${environment.apiUrl}/reports/overview`).pipe(
        catchError(error => of(null))
      ),
      waitlist: this.http.get(`${environment.apiUrl}/receivers/statistics/waitlist`).pipe(
        catchError(error => of(null))
      ),
      transplants: this.http.get(`${environment.apiUrl}/transplant-procedures/statistics`).pipe(
        catchError(error => of(null))
      ),
      organStats: this.http.get(`${environment.apiUrl}/organs/statistics`).pipe(
        catchError(error => of(null))
      ),
      upcoming: this.http.get(`${environment.apiUrl}/transplant-procedures/upcoming`).pipe(
        catchError(error => of([]))
      )
    }).subscribe(results => {
  ...
  updateBloodTypeChart(): void {
    if (this.waitlistStats && this.waitlistStats.byBloodType) {
      ...
      const counts = bloodTypes.map(type => this.waitlistStats.byBloodType[type]);
  ...
  updateStatusChart(): void {
    if (this.transplantStats && this.transplantStats.byOutcome) {
      ...
      const counts = outcomes.map(outcome => this.transplantStats.byOutcome[outcome]);
